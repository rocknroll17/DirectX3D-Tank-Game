...
public:
	...
	void tankUpdate(float timeDiff, vector<CObstacle>& obstacles, Tank& otank, CWall* walls)
	{
		if (!created) return;
		const float TIME_SCALE = 3.3;
		D3DXVECTOR3 cord = this->getCenter();
		...
		/*
		for (int i = 0; i < 4; i++) {
			if (tank_part[0].hasIntersected(walls[i])) {
				tX = cord.x;
				tZ = cord.z;
				break;
			}
